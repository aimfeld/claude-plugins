...
- **Summary Stats table** adds one "Test suite run — {suite}" row per suite (e.g. backend, frontend) with the exact command used + where it came from. The existing "Test coverage" row remains for single-suite projects or as an aggregate when one exists. ([#TBD](https://github.com/aimfeld/claude-plugins/pulls))
